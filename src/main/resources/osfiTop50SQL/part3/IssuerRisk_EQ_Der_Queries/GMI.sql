...
WHERE
	...
	AND POS.RUN_ID=(select max(run_id)from sa_ccr.positn where bus_dt=to_date(?,'YYYY-MM-DD') and expr_run_id='99999999'
	 and SRC_SYS_CD like '%GMI%' )
	AND POS.SRC_SYS_CD IN ('GMI')
	AND POS.PRODUCT_ID IN ('40','46')
)	
...
SELECT 
	...
	T1.PRODUCT_ID, T1.PRODUCT, T1.OPT_TYPE, T1.NOTIONAL, T1.UNITS, decode(nvl(T1.CONTRACT_SIZE,0),0,1,
	T1.CONTRACT_SIZE) as CONTRACT_SIZE, T1.STRIKE, T1.STRIKE_CCY,
	...
	RPT.CCP_IND, T1.PRODUCT_CD, T1.ASSET_CLASS,'GMI' as SOURCE
FROM 
	T1
 JOIN SA_CCR.RPT_CCBASEL_REPORT RPT ON T1.KEY_ID=RPT.TRADE_ID AND T1.SRC_SYS_CD=RPT.SRC_SYS_CD AND T1.BUS_DT=RPT.BUS_DT
WHERE
