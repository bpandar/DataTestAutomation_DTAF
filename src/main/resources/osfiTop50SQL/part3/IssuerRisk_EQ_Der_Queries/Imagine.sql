...
WHERE
  ...
  AND POS.RUN_ID in (select max(run_id)from sa_ccr.positn where bus_dt=TO_DATE(?, 'YYYY-MM-DD') and expr_run_id='99999999' and SRC_SYS_CD='IMAGINE' )
...
SELECT 
	...
	T1.PRODUCT_CD, T1.ASSET_CLASS, 'IMAGINE' as SOURCE
FROM 
	T1
JOIN SA_CCR.RPT_CCBASEL_REPORT RPT 
ON T1.KEY_ID=RPT.TRADE_ID AND T1.SRC_SYS_CD=RPT.SRC_SYS_CD AND T1.BUS_DT=RPT.BUS_DT 
WHERE 
