...
SELECT 
	...
	END) AS BUY_SELL,	
		...
		--'' AS REF_TYPE, '' AS REF_PRICE, '' AS REF_CCY,
		EQ.SEC_TYPE AS REF_TYPE, EQ.PRICE AS REF_PRICE, EQ.CCY_CD AS REF_CCY,
	PD.PRODUCT_CD,
	PD.ASSET_CLASS
FROM
	SA_CCR.POSITN POS
	LEFT JOIN SA_CCR.MKT_EQ_DATA EQ ON (REPLACE(POS.REF_ENTITY_NAME, '.OQ', '.O')=EQ.SEC_SYMBOL AND POS.BUS_DT=EQ.BUS_DT)
...
WHERE
	POS.BUS_DT= TO_DATE(?, 'YYYY-MM-DD') -- Should be parametrized.
	AND POS.RUN_ID=(select max(run_id)from sa_ccr.positn where bus_dt=to_date(?,'YYYY-MM-DD')
	and expr_run_id='99999999' and PRODUCT_ID IN ('33','34')
    )
	AND POS.PRODUCT_ID IN ('33','34')
	AND POS.REF_ENTITY_NAME NOT LIKE '%bsk%'
)	
...
SELECT 
	...
	T1.PRODUCT_CD, T1.ASSET_CLASS, 'TRADEREH' as SOURCE
FROM 
	T1
 JOIN SA_CCR.RPT_CCBASEL_REPORT RPT ON T1.KEY_ID=REGEXP_SUBSTR(RPT.TRADE_ID, '[^ _]+') AND T1.BUS_DT=RPT.BUS_DT
WHERE 
	RPT.INTRNL_EXTR_IND='N'
	AND RPT.RUN_ID in (select max(run_id)from sa_ccr.rpt_ccbasel_report where bus_dt=to_date(?,'YYYY-MM-DD'))
	AND RPT.SRC_SYS_CD  IN ('NONCORE_IB', 'TRADEREH')