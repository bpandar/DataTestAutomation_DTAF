...
WHERE
	...
	AND POS.RUN_ID=(select max(RUN_ID) from SA_CCR.POSITN where BUS_DT= TO_DATE(?, 'YYYY-MM-DD') and
     PRODUCT_ID IN ('27','35','37')
    )
	--POS.BUS_DT='31-JAN-2018' AND POS.RUN_ID='318'
	--AND POS.SRC_SYS_CD IN ('NONCORE_IBG')
	AND POS.PRODUCT_ID IN ('27','35','37')
)	SELECT
