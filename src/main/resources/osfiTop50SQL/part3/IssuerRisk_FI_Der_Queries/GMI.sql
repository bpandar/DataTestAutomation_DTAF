...
SELECT 
	...
	END) AS BUY_SELL,	
		...
		BBG.UNDRLY_SPOT_TKER, 
	(CASE 
		WHEN FUT_CAT='Index' THEN 'I'
		WHEN FUT_CAT='Single' THEN 'S'
	ELSE ''
	END) AS REF_TYPE,
	BBG.EOD_PRICE AS REF_PRICE,
	BBG.UNDRLY_CCY_CD AS REF_CCY,
	PD.PRODUCT_CD,
	PD.ASSET_CLASS
FROM
	SA_CCR.POSITN POS
...
WHERE
	...
	AND POS.RUN_ID in (select max(run_id)from sa_ccr.positn where bus_dt=to_date(?,'YYYY-MM-DD') and expr_run_id='99999999' and SRC_SYS_CD like '%GMI%' ) -- Should be parametrized
	--POS.BUS_DT='31-JAN-2018' AND POS.RUN_ID='318'
	--AND POS.SRC_SYS_CD IN ('GMI')
	AND POS.PRODUCT_ID IN ('64')
)	
...
SELECT 
	...
	T1.PRODUCT_CD, T1.ASSET_CLASS, 'GMI' as SOURCE
FROM 
	T1
LEFT JOIN SA_CCR.RPT_CCBASEL_REPORT RPT ON T1.KEY_ID=RPT.TRADE_ID AND T1.SRC_SYS_CD=RPT.SRC_SYS_CD AND T1.BUS_DT=RPT.BUS_DT
WHERE
 RPT.RUN_ID in (select max(run_id)from sa_ccr.rpt_ccbasel_report where bus_dt=to_date(?,'YYYY-MM-DD')) and
