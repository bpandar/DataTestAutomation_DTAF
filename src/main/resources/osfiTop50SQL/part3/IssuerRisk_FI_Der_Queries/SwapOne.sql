...
WHERE
	...
	AND POS.RUN_ID=(
	select max(RUN_ID) from SA_CCR.POSITN where BUS_DT= TO_DATE(?, 'YYYY-MM-DD') and SRC_SYS_CD = 'SWAPONE'
    AND POS.PRODUCT_ID IN ('69','70')
    )
	AND POS.PRODUCT_ID IN ('69','70')
)	
...
SELECT 
	...
	T1.PRODUCT_CD, T1.ASSET_CLASS, 'SWAPONE_FI' as SOURCE
FROM 
	T1
